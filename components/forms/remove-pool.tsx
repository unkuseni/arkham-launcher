...
import { type RemoveFromCPMMPoolParams, removeFromCPMMPool } from "@/lib/liquidity/cpmm/remove";
import useUmiStore, { ConnectionStatus } from "@/store/useUmiStore";
import { Network } from "@/store/useUmiStore";
import { zodResolver } from "@hookform/resolvers/zod";
import BN from "bn.js";
import { AlertCircle, ArrowDown, DollarSign, Droplets, Minus, RefreshCw, Settings2, Trash2 } from "lucide-react";
...
const RemoveLiquidity = () => {
  ...
  useEffect(() => {
    const calculateWithdrawal = () => {
      ...
      const lpAmountToRemove = Number(selectedPosition.lpAmount) * percentage / 1e9;
    ...
    return (
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          >
            <RefreshCw className={`h-3 w-3 mr-1 ${loadingPositions ? 'animate-spin' : ''}`} />
            Refresh
          </Button>
        </div>
        ...
        {signer && connectionStatus === ConnectionStatus.CONNECTED ? (
                  ...
                  <div className="flex items-center justify-between w-full">
                    <div className="flex flex-col items-start">
                      ...
                      <span className="text-xs text-muted-foreground">
                        {position.tokenA.amount} {position.tokenA.symbol} + {position.tokenB.amount} {position.tokenB.symbol}
                      </span>
                    </div>
                    <div className="flex flex-col items-end ml-4">
                      <span className="text-sm font-mono">{position.formattedAmount} LP</span>
                      {position.value && (
                        <span className="text-xs text-green-600">{position.value}</span>
                      )}
                    </div>
                  </div>
                </SelectItem>
          ...
          <div className="text-sm text-muted-foreground p-3 border rounded-md bg-muted/50">
            {!signer ? "Connect your wallet to see LP positions" : "Connecting..."}
          </div>
        )}

        {selectedPosition && (
          <div className="text-xs text-muted-foreground space-y-1">
            <div>Pool: {selectedPosition.poolId.slice(0, 8)}...{selectedPosition.poolId.slice(-8)}</div>
      ...
      <div className="min-h-screen bg-gradient-to-br from-background via-background/95 to-secondary/10 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 py-12 max-w-4xl relative">
          <div className="text-center mb-8">
            ...
            <h1 className="text-4xl font-bold text-orange-600 mb-4">Liquidity Removed Successfully!</h1>
            <p className="text-lg text-muted-foreground">Your LP tokens have been withdrawn from the pool</p>
          </div>

          <Card className="border-orange-200 bg-orange-50/50 dark:bg-orange-950/20 dark:border-orange-800">
            <CardHeader className="text-center">
              <CardTitle className="text-orange-700 dark:text-orange-300">Transaction Details</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  <Label className="text-sm font-medium text-muted-foreground">LP Amount Removed</Label>
      ...
      <div className="container mx-auto px-4 py-12 max-w-4xl relative">
                    ...
                    <div className="p-4 bg-muted/50 rounded-lg">
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                          ...
                          <p className="font-mono">{selectedPosition.tokenA.amount} {selectedPosition.tokenA.symbol}</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground">Token B</p>
                          <p className="font-mono">{selectedPosition.tokenB.amount} {selectedPosition.tokenB.symbol}</p>
                        </div>
                      </div>
                    </div>
                  )}
                    ...
                    {estimatedWithdrawal && selectedPosition && (
                      <div className="p-4 bg-muted/50 rounded-lg">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            <p className="text-xs text-muted-foreground">{selectedPosition.tokenA.symbol}</p>
                            <p className="text-lg font-mono">{estimatedWithdrawal.tokenA}</p>
                          </div>
                          <div>
                            <p className="text-xs text-muted-foreground">{selectedPosition.tokenB.symbol}</p>
                            <p className="text-lg font-mono">{estimatedWithdrawal.tokenB}</p>
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    render={({ field }) => (
                      <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                        <div className="space-y-0.5">
                          ...
                          <FormDescription>
                            Automatically close wrapped SOL account after withdrawal
                          </FormDescription>
                        </div>
                        <FormControl>
                          <Checkbox
        ...
        {!umi?.identity && (
          <Alert className="max-w-2xl mx-auto mt-6">
            ...
            <AlertDescription>
              Please connect your wallet to remove liquidity. Make sure you have LP tokens and SOL for transaction fees.
            </AlertDescription>
          </Alert>
        )}
