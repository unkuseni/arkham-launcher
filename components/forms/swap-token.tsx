...
import { AlertCircle, ArrowDown, ArrowLeftRight, ArrowUpDown, DollarSign, Droplets, RefreshCw, Settings2, TrendingUp } from "lucide-react";
...
const SwapToken = () => {
  ...
  const [activeTab, setActiveTab] = useState<"exact_in" | "exact_out">("exact_in");
  ...
  useEffect(() => {
    const calculateEstimates = () => {
      ...
      if (!inputAmount || inputAmount <= 0 || !inputMint || !outputMint || inputMint === outputMint) {
        setEstimatedOutput("");
        setPriceImpact(null);
        return;
      }
    ...
    calculateEstimates();
  }, [watchedValues.inputAmount, watchedValues.inputMint, watchedValues.outputMint, watchedValues.swapMode]);
  ...
  const TokenSelector = ({
    ...
    const filteredTokens = availableTokens.filter(token => token.mint !== excludeMint);
  ...
  const handleSubmit = async (data: SwapFormData) => {
    ...
    try {
      ...
      if (activeTab === "exact_in") {
        ...
        const inputToken = availableTokens.find(t => t.mint === data.inputMint);
        ...
        const params: SwapParams = {
          ...
          inputAmountParam: new BN(Math.floor(data.inputAmount * 10 ** inputDecimals)),
        ...
        const outputToken = availableTokens.find(t => t.mint === data.outputMint);
        ...
        const params: SwapBaseOutParams = {
          ...
          signer: umi.identity,
          poolIdParam: data.poolId || undefined,
          outputAmountParam: new BN(Math.floor((data.outputAmount || 0) * 10 ** outputDecimals)),
  ...
  const setMaxAmount = () => {
    ...
    const selectedToken = availableTokens.find(token => token.mint === inputMint);
      ...
      <div className="min-h-screen bg-gradient-to-br from-background via-background/95 to-secondary/10 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 py-12 max-w-4xl relative">
          <div className="text-center mb-8">
            ...
            <h1 className="text-4xl font-bold text-blue-600 mb-4">Swap Completed!</h1>
            <p className="text-lg text-muted-foreground">Your tokens have been successfully swapped</p>
          </div>

          <Card className="border-blue-200 bg-blue-50/50 dark:bg-blue-950/20 dark:border-blue-800">
            <CardHeader className="text-center">
              <CardTitle className="text-blue-700 dark:text-blue-300">Transaction Details</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  <Label className="text-sm font-medium text-muted-foreground">Input Amount</Label>
                  <div className="p-3 bg-background rounded-lg border">
                    {(Number(result.inputAmount) / 1e9).toFixed(9)} tokens
                  </div>
                </div>
                <div className="space-y-2">
                  <Label className="text-sm font-medium text-muted-foreground">Output Amount</Label>
                  <div className="p-3 bg-background rounded-lg border">
                    {(Number(result.outputAmount) / 1e9).toFixed(9)} tokens
                  </div>
                </div>
                <div className="space-y-2">
                  <Label className="text-sm font-medium text-muted-foreground">Trade Fee</Label>
                  <div className="p-3 bg-background rounded-lg border">
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background/95 to-secondary/10 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 py-12 max-w-4xl relative">
        ...
        <Card className="max-w-2xl mx-auto">
          ...
          <CardContent>
            <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "exact_in" | "exact_out")}>
              <TabsList className="grid w-full grid-cols-2 mb-6">
                <TabsTrigger value="exact_in" className="flex items-center gap-2">
                  <ArrowDown className="w-4 h-4" />
                  Exact Input
                </TabsTrigger>
                <TabsTrigger value="exact_out" className="flex items-center gap-2">
              ...
              <Form {...form}>
                <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="Enter specific pool ID to swap from..." {...field} />
                          </FormControl>
                          <FormDescription>
                            Leave empty to auto-select the best pool based on token pair
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                      ...
                      render={({ field }) => (
                        <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                          <div className="space-y-0.5">
                            <FormLabel className="text-base">Auto-select Best Pool</FormLabel>
                            <FormDescription>
                              Automatically find the best pool for your token pair when no pool ID is specified
                            </FormDescription>
                          </div>
                          <FormControl>
                            <Checkbox
                    ...
                    {!watchedValues.poolId && (
                      <div className="p-3 bg-blue-50 dark:bg-blue-950/20 rounded-lg border border-blue-200 dark:border-blue-800">
                        ...
                        <p className="text-xs text-blue-600 dark:text-blue-400">
                          The system will automatically find the {watchedValues.autoSelectPool ? 'best available' : 'first available'} pool for your selected token pair
                        </p>
                      </div>
                    )}
                  </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        <TokenSelector
                          ...
                          onValueChange={(value) => form.setValue("inputMint", value)}
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem className="mt-2">
                              <FormControl>
                                <Input placeholder="Or enter token mint address..." {...field} />
                              </FormControl>
                              <FormMessage />
                            </FormItem>
                          )}
                      ...
                      <div>
                        <TokenSelector
                          ...
                          onValueChange={(value) => form.setValue("outputMint", value)}
                      ...
                      {(watchedValues.inputMint && watchedValues.outputMint && watchedValues.inputMint !== watchedValues.outputMint) && (
                        <div className="p-3 bg-muted/50 rounded-lg border">
                          ...
                          {watchedValues.poolId ? (
                            <div className="space-y-1 text-xs text-muted-foreground">
                              <div>Pool ID: {watchedValues.poolId.slice(0, 8)}...{watchedValues.poolId.slice(-8)}</div>
                              <div>Using specified pool for swap</div>
                            </div>
                          ) : (
                            <div className="space-y-1 text-xs text-muted-foreground">
                              <div>Pool will be auto-selected based on token pair</div>
                              <div>Mode: {watchedValues.autoSelectPool ? 'Best pool by liquidity' : 'First available pool'}</div>
                            </div>
                          )}
                        </div>
                      )}
                  ...
                  <div className="space-y-4">
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>Pool ID (Optional)</FormLabel>
                          <FormControl>
                            <Input placeholder="Leave empty for auto-selection..." {...field} />
                          </FormControl>
                          <FormDescription>
                            Specify a pool ID or let the system find the best pool
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <label htmlFor="show-advanced-swap" className="text-sm font-medium cursor-pointer flex items-center gap-2">
                  ...
                  <Button
                    type="submit"
                    className="w-full"
                    disabled={isSubmitting || !umi?.identity || !watchedValues.inputMint || !watchedValues.outputMint}
                  >
                    {isSubmitting ? (
                      <>
                        <LoadingSpinner className="mr-2 h-4 w-4" />
                        {activeTab === "exact_in" ? "Swapping..." : "Calculating & Swapping..."}
                      </>
                    ) : (
                      <>
                        <ArrowLeftRight className="mr-2 h-4 w-4" />
                        {activeTab === "exact_in" ? "Swap Tokens" : "Swap for Exact Amount"}
                      </>
                    )}
                  </Button>
                </form>
        ...
        {!umi?.identity && (
          <Alert className="max-w-2xl mx-auto mt-6">
            ...
            <AlertDescription>
              Please connect your wallet to swap tokens. Make sure you have sufficient tokens and SOL for transaction fees.
            </AlertDescription>
          </Alert>
        )}
      </div>
